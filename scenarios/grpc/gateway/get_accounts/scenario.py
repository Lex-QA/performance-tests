...
class GetAccountsTaskSet(GatewayGRPCTaskSet):
    ...
    3. Получает список всех счетов для текущего пользователя.
    ...
    def get_accounts(self):
        """
        Получаем список всех счетов, если счёт был успешно открыт.
        """
        if not self.create_user_response:
            return
...
class GetAccountsScenarioUser(LocustBaseUser):
    """
    Пользователь Locust, исполняющий последовательный сценарий открытия и получения счетов.
    """
